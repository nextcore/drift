...
func (s *storageState) Build(ctx core.BuildContext) core.Widget {
	...
	return demoPage(ctx, "Storage",
		...
		widgets.NewButton("Pick File", func() {
			s.pickFile()
		}).WithColor(colors.Primary, colors.OnPrimary),
		widgets.VSpace(12),

		widgets.NewButton("Pick Directory", func() {
			s.pickDirectory()
		}).WithColor(colors.Secondary, colors.OnSecondary),
		widgets.VSpace(12),

		widgets.NewButton("Save File", func() {
			s.saveFile()
	...
	if file == nil && path == "" {
		return widgets.NewContainer(
			widgets.PaddingAll(16,
				widgets.TextOf("No item selected", rendering.TextStyle{
			...
			widgets.PaddingAll(16,
				widgets.Column{
					...
					ChildrenWidgets: []core.Widget{
						...
						widgets.VSpace(4),
						widgets.TextOf(file.Path, rendering.TextStyle{
	...
	return widgets.NewContainer(
		widgets.PaddingAll(16,
			widgets.Column{
				...
				MainAxisSize:       widgets.MainAxisSizeMin,
				ChildrenWidgets: []core.Widget{
					widgets.TextOf("Directory:", rendering.TextStyle{
						Color:      colors.OnSurfaceVariant,
						FontSize:   12,
						FontWeight: rendering.FontWeightBold,
					}),
...
func (s *storageState) appDirectoriesCard(colors theme.ColorScheme) core.Widget {
	...
	if len(dirs) == 0 {
		return widgets.NewContainer(
			widgets.PaddingAll(16,
				widgets.TextOf("Loading directories...", rendering.TextStyle{
					Color:    colors.OnSurfaceVariant,
					FontSize: 14,
				}),
			),
